            ...
            ...(function buildReasonDatasets() {
                ...
                if (!currentTacticPrefix) {
                    ...
                    return displayCols.filter(c => c !== 'O_UNIT' && c !== 'O_SALE').map((c) => {
                        ...
                        const anomalyMatches = timeSeriesData.map(pt => {
                            ...
                            return matchingAnomaly;
                        });

                        return {
                            type: 'scatter',
                            label: `${c} Anomalies`,
                            data: anomalyMatches.map((m, i) => m && timeSeriesData[i][c] !== undefined ? timeSeriesData[i][c] : null),
    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                {(setTacticFilter || setSeverityFilter) ? (
                    <div className="flex items-center gap-2">
                        {setTacticFilter && availableTactics && availableTactics.length > 0 && (
                            <select
                        ...
                        {setSeverityFilter && availableSeverities && availableSeverities.length > 0 && (
                            <select
                                className="bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded focus:ring-blue-500 focus:border-blue-500 block p-1.5"
                        ...
                        {setTypeFilter && availableTypes && availableTypes.length > 1 && (
                            <select
                                className="bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded focus:ring-blue-500 focus:border-blue-500 block p-1.5"
                                value={activeTypeFilter}
                                onChange={(e) => setTypeFilter(e.target.value)}
                            >
                                {availableTypes.map(t => <option key={t} value={t}>{t === 'All' ? 'All Types' : t.replace(/_/g, ' ')}</option>)}
                            </select>
                        )}
                    </div>
                ) : <div />}
