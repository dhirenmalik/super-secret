"""Converted and modularized logic from:
reference_codes/02 Exclude Flag Analysis/01 Code (QA Output Generation)/code01_2_Category_QAandSubcat_Updated.ipynb

This module preserves the core notebook transformations while exposing
function-level interfaces used by phase services.
"""
...
def normalize_aggregate_df(raw_df: pd.DataFrame) -> pd.DataFrame:
    """Apply notebook cleaning and aggregation steps to uploaded aggregate csv."""
    ...
    missing = [col for col in required if col not in df_agg.columns]
    if missing:
    ...
    # Notebook logic: create a deterministic brand->advertiser map.
    br_adv_ls: Dict[str, str] = {}
    for i in range(len(df_agg)):
        brand = df_agg.iloc[i]["UNIQUE_BRAND_NAME"]
        adv = df_agg.iloc[i]["UNIQUE_ADV_NAME"]
        if brand not in br_adv_ls and adv != "N/A" and adv != "0":
            br_adv_ls[brand] = adv
    ...
    df_agg = df_agg.drop("UNIQUE_ADV_NAME", axis=1)
    df_agg = df_agg.merge(br_adv_df, how="left", on="UNIQUE_BRAND_NAME")
...
def build_sub_dis_candidates(df_agg: pd.DataFrame) -> pd.DataFrame:
    ...
    sub_sum["AVG_PRICE"] = sub_sum["O_SALE"] / sub_sum["O_UNIT"].replace(0, np.nan)
    sub_sum["AVG_PRICE"] = sub_sum["AVG_PRICE"].fillna(0).round(2)
    ...
    sub_percent = sub_percent.rename(
        columns={c: f"{c}_PCT" for c in sub_percent.columns if c != "L2"}
    )
...
def _is_same_name(str1: str, str2: str) -> bool:
    if str1 == "0" or str2 == "0":
        return False

    mutated_str1 = "".join(str1.split())
    mutated_str2 = "".join(str2.split())
    if mutated_str1 == mutated_str2:
        return True
    ...
    connectors = ["&", "N", "n", "AND", "and", ""]
    parts_of_str1 = str1.split()
    parts_of_str2 = str2.split()

    if (len(parts_of_str1) in {2, 3}) and (len(parts_of_str2) in {2, 3}):
        if parts_of_str1[0] == parts_of_str2[0] and parts_of_str1[-1] == parts_of_str2[-1]:
            connector_of_str1 = parts_of_str1[1] if len(parts_of_str1) == 3 else ""
            connector_of_str2 = parts_of_str2[1] if len(parts_of_str2) == 3 else ""
            if connector_of_str1 in connectors and connector_of_str2 in connectors:
                return True
...
def exclude_combine_flags(
    ...
    df_brand_adv_agg = (
        aggregate_data.groupby(["UNIQUE_ADV_NAME", "UNIQUE_BRAND_NAME"], as_index=False)
    ...
    df_brand_agg["Total_Spend"] = (
        df_brand_agg["M_SEARCH_SPEND"] + df_brand_agg["M_TOTAL_DISPLAY_SUM_SPEND"]
    )
    df_brand_agg["Spends_Sales_Ratio"] = df_brand_agg["Total_Spend"] / df_brand_agg["O_SALE"].replace(0, np.nan)
    df_brand_agg["Spends_Sales_Ratio"] = df_brand_agg["Spends_Sales_Ratio"].replace([np.inf, -np.inf], np.inf).fillna(np.inf)
    ...
    df_brand_agg.loc[
        (df_brand_agg["Spends_Sales_Ratio"] == 0)
        | (df_brand_agg["Spends_Sales_Ratio"] == np.inf),
        "Exclude_Flag",
    ] = 1
    df_brand_agg.loc[df_brand_agg["Spends_Sales_Ratio"] > 15, "Exclude_Flag"] = 1

    # Keep combine flag behavior aligned with current notebook state.
    df_brand_agg["Combine_Flag"] = np.nan

    normalized_private_brands = {str(name).upper() for name in private_brand_list}
    df_brand_agg.loc[
        df_brand_agg["UNIQUE_BRAND_NAME"].astype(str).str.upper().isin(normalized_private_brands),
        "Exclude_Flag",
    ] = 1
    ...
    df_brand_adv_agg.loc[
        df_brand_adv_agg["UNIQUE_ADV_NAME"].isin(adv_exclusion_list), "Exclude_Flag"
    ] = 1
    df_brand_adv_agg["Spend_Brand"] = (
        df_brand_adv_agg["M_SEARCH_SPEND"] + df_brand_adv_agg["M_TOTAL_DISPLAY_SUM_SPEND"]
    )
...
def build_final_flag_df(
    ...
    irrelevantsub["Spend_Brand"] = (
        irrelevantsub["M_SEARCH_SPEND"] + irrelevantsub["M_TOTAL_DISPLAY_SUM_SPEND"]
    )
